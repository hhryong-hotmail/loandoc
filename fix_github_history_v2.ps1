...
# git log를 UTF-8 파일로 저장
$logFile = "D:\LoanDoc\git_log_utf8.txt"
& git -c core.quotepath=false log --format="%H|%an|%ad|%s" --date=iso -34 --encoding=UTF-8 | Out-File -FilePath $logFile -Encoding UTF8

# 파일을 UTF-8로 읽기
$commits = Get-Content -Path $logFile -Encoding UTF8
...
foreach ($commit in $commits) {
    ...
    # 커밋 해시로 기존 레코드 찾아서 업데이트
    $sql = @"
UPDATE github_history 
SET program_name = E'$programNameEscaped',
    change_reason = E'$messageEscaped',
    developer_name = E'$authorEscaped',
    important_code_content = E'$programNameEscaped'
WHERE repo_name = 'hhryong-hotmail/loandoc' 
  AND change_datetime = '$pgDate';
"@
    
    Write-Host "Updating: $hash - $message"
    $sqlFile = "D:\LoanDoc\temp_update.sql"
    $sql | Out-File -FilePath $sqlFile -Encoding UTF8
    
    $output = & $psql -U $user -d $dbName -c "SET client_encoding TO 'UTF8';" -f $sqlFile 2>&1
