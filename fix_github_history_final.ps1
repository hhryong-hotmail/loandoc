# GitHub 히스토리 한글 수정 스크립트 (최종 - 코드 페이지 설정)
...
foreach ($hash in $hashes) {
    ...
    # 커밋 정보 가져오기 (코드 페이지 65001로 설정)
    $commitInfo = cmd /c "chcp 65001 >nul && git log -1 --format=`"%H|%an|%ad|%s`" --date=iso $hash"
...
# SQL 파일로 저장
$sqlFile = "D:\LoanDoc\update_github_history_final.sql"
